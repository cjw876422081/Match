...
void Input( int [] , QJ_t  [] , int , int ) ;
void Output( int [] , QJ_t  [] , int ) ;
int main(void)
{
	int n , q ; 
	scanf("%d%d" , & n , & q ) ; 
...
void Output( int sz[] , QJ_t cx[] , int q )
{
	int i , j ; 
	for( i = 0 ; i < q ; i ++ )
	{
		int min = sz[cx[i].left] ; 
		for( j = cx[i].left + 1 ; j <=cx[i].right ; j ++ )
		{  
		    if(sz[j] < min )
			{  
		        min = sz[j] ;  
...
void Input( int sz[] , QJ_t cx[] , int n , int q )
{
	int i ; 
	for( i = 0 ; i < n ; i ++ )
	{
	...
	for( i = 0 ; i < q ; i ++ )
	{
		scanf("%d%d" , &cx[i].left , &cx[i].right ) ; 
	}
}
#endif
